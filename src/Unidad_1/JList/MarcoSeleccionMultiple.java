package Unidad_1.JList;
// Fig. 1.20: MarcoSeleccionMultiple.java
...
public class MarcoSeleccionMultiple extends JFrame{
    ...
    //Obtiene la lista de los valores elegidos, y crea una segunda List para guardar esos valores
    List<String> toSave = new ArrayList<>(), saved = new ArrayList<>();
        ...
        botonJButtonCopiar.addActionListener(
                new ActionListener(){ // clase interna anónima
                    ...
                    public void actionPerformed(ActionEvent evento){
                        toSave = listaJListColores.getSelectedValuesList();
                        //Si el valor elegido no ha sido elegido anteriormente, lo guarda en saved, si ya
                        //fue elegido anteriormente, lo quita de saved.
                        for(String color : toSave){
                            if(saved.contains(color)){
                                saved.remove(color);
                            }else saved.add(color);
                        }
                        // coloca los valores seleccionados en listaJListCopia
                        listaJListCopia.setListData(saved.toArray(new String[0]));
                    }
                }
        );
...
} // fin de la clase MarcoSeleccionMultip
